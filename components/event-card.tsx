...
export function EventCard({ event, index = 0 }: { event: PublicEvent; index?: number }) {
  ...
  return <Link href={`/events/${event.slug}`} className="event-card glass-brutalist group block overflow-hidden rounded-[1.75rem] transition duration-500 active:scale-[.99] md:rounded-3xl md:hover:-translate-y-1.5 md:hover:border-emerald-500/50">
    <div className={`event-card-media relative h-60 overflow-hidden bg-gradient-to-br ${gradient} p-5 md:h-52`}>
      ...
      <span className="relative z-10 rounded-xl border border-white/20 bg-black/50 px-3 py-1.5 text-[10px] font-bold tracking-[.22em] text-emerald-300 backdrop-blur">{(event.category || "EVENT").toUpperCase()}</span>
      <span className="absolute right-5 top-5 z-10 grid h-10 w-10 place-items-center rounded-xl border border-white/25 bg-black/50 text-emerald-300 opacity-0 backdrop-blur transition group-hover:opacity-100"><ArrowUpRight size={16}/></span>
    </div>
    <div className="event-card-body relative z-10 p-5 md:p-6">
      <div className="flex justify-between gap-5">
        ...
        <div className="shrink-0 text-right">
          <p className="text-sm font-bold text-emerald-400">{formatDate(event.startAt)}</p>
      ...
      <div className="mt-6 flex items-center justify-between border-t border-white/[.075] pt-4 text-[10px] tracking-[.18em] text-white/38">
        ...
        <span className={event.registrationOpen ? "rounded-xl border border-black bg-[#00FF66] px-3 py-1 font-bold text-black shadow-[2px_2px_0px_0px_rgba(255,255,255,0.8)]" : "rounded-xl border border-white/20 bg-white/5 px-3 py-1.5 text-white/48"}>{event.registrationOpen ? "OPEN" : event.status.toUpperCase()}</span>
      </div>
    </div>
  </Link>;
}
