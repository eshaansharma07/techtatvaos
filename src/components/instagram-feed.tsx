  ...
  return (
    <section className="border-t border-white/[0.06] bg-black py-20 md:py-28 relative overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl xl:max-w-[1380px] 2xl:max-w-[1536px] px-5 md:px-6 relative">
        ...
        <Reveal delay={0.04}>
          <div className="mb-14 relative rounded-[2.2rem] glass-brutalist p-8 md:p-10 shadow-[0_0_40px_rgba(16,185,129,0.06)] overflow-hidden flex flex-col lg:flex-row items-center justify-between gap-10">
            
            {/* Technical HUD Crosshairs / Corner Brackets */}
            <div className="absolute top-4 left-4 w-3.5 h-3.5 border-t border-l border-emerald-500/40 pointer-events-none" />
            <div className="absolute top-4 right-4 w-3.5 h-3.5 border-t border-r border-emerald-500/40 pointer-events-none" />
            <div className="absolute bottom-4 left-4 w-3.5 h-3.5 border-b border-l border-emerald-500/40 pointer-events-none" />
            <div className="absolute bottom-4 right-4 w-3.5 h-3.5 border-b border-r border-emerald-500/40 pointer-events-none" />
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left z-10 select-none">
              <div className="relative shrink-0">
                ...
                <div className="absolute inset-[-6px] rounded-full border border-dashed border-emerald-500/30 animate-[spin_20s_linear_infinite]" />
                {/* Glowing reactor avatar circle */}
                <div className="absolute inset-[-3px] rounded-full bg-emerald-500" />
                <div className="relative grid h-16 w-16 place-items-center rounded-full bg-[#0c0c0e] text-white">
                  <Instagram size={28} className="text-emerald-400 drop-shadow-[0_0_10px_rgba(0,255,102,0.4)] animate-pulse" />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2 justify-center sm:justify-start">
                  ...
                  <span className="grid h-5 w-5 place-items-center rounded-full bg-emerald-500 text-black shadow-[0_0_10px_rgba(0,255,102,0.3)]">
                    <svg viewBox="0 0 20 20" fill="currentColor" className="h-3 w-3">
                      <path fillRule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clipRule="evenodd" />
                    </svg>
                  </span>
                </div>
                <span className="self-center sm:self-start inline-flex rounded-full border border-emerald-500/20 bg-emerald-500/10 px-3 py-0.5 text-[8px] font-black tracking-[0.2em] text-emerald-400 uppercase">
            ...
            <div className="grid grid-cols-3 gap-6 sm:gap-12 w-full lg:w-auto z-10">
              ...
              <div className="flex flex-col items-center lg:items-end text-center lg:text-right group relative">
                <span className="text-[10px] font-black tracking-[0.2em] font-mono text-white/35 group-hover:text-emerald-400 transition-colors uppercase">
                  POSTS
                </span>
                <span className="text-4xl sm:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-white/70 tracking-tight font-mono mt-2 drop-shadow-[0_0_12px_rgba(0,255,102,0.15)] group-hover:scale-105 transition-transform duration-300">
                  {stats?.postsCount || "40"}
                </span>
                  ...
                  {Array.from({ length: 8 }).map((_, idx) => (
                    <div 
                      ...
                      className={`h-1.5 w-2.5 rounded-sm transition-all duration-500 ${
                        idx < 3 
                          ? "bg-emerald-500 shadow-[0_0_6px_#00FF66]" 
                          : "bg-white/[0.04]"
                      }`} 
                    />
                  ))}
