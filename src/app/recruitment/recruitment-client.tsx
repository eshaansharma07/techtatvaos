        ...
        <aside className="lg:sticky lg:top-28">
          <div className="glass-brutalist rounded-[2rem] p-6 md:p-8">
            ...
            <div className="mt-7 grid grid-cols-3 gap-3">
              {["Teams", "Roles", "Questions"].map((label, index) => <div className="glass-brutalist px-4 py-4 rounded-xl text-center" key={label}><p className="text-2xl font-bold tracking-[-.04em] text-white">{[data.teams.length, data.roles.length, data.questions.length][index]}</p><p className="mt-1 text-[10px] uppercase tracking-[.16em] text-white/35">{label}</p></div>)}
            </div>
          </div>
        </aside>
 
        <form onSubmit={submit} className="glass-brutalist min-h-[680px] rounded-[2rem] p-4 md:p-6">
...
function QuestionPanel({ questions, answers, setAnswers, uploadFile, files, uploading }: any) {
  return <div><h2 className="text-3xl font-bold tracking-[-.04em] text-white">Team questions.</h2><div className="mt-5 grid gap-4">{questions.map((question: Question) => <label className="block rounded-2xl border border-white/[.08] bg-white/[.03] p-4 text-sm text-white/70" key={question.id}><span className="font-bold">{question.label}{question.required ? <span className="text-rose-200"> *</span> : null}</span>{question.helpText ? <span className="mt-1 block text-xs leading-5 text-white/38">{question.helpText}</span> : null}<QuestionInput question={question} value={answers[question.id]} onChange={(value: any) => setAnswers((state: any) => ({ ...state, [question.id]: value }))} uploadFile={uploadFile} files={files} uploading={uploading} /></label>)}</div>{!questions.length ? <p className="rounded-2xl border border-white/[.08] bg-white/[.03] p-6 text-white/45">No extra questions for this role. Continue to review.</p> : null}</div>;
}
...
function GithubPanel({ form, update, error, setError }: { form: Record<string, any>; update: (name: string, value: any) => void; error?: string; setError: (err: string) => void }) {
  ...
  return (
    <div>
      <h2 className="text-3xl font-bold tracking-[-.04em] text-white">GitHub Profile.</h2>
...
function ReviewPanel({ form, team, role, questions, answers, files, onEdit }: any) {
  ...
  return <div><div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-3xl font-bold tracking-[-.04em] text-white">Review everything.</h2><button type="button" onClick={() => onEdit(1)} className="brutalist-btn-dark inline-flex items-center gap-2 rounded-xl px-4 py-2 text-xs"><Pencil size={13} /> Edit details</button></div><div className="mt-5 grid gap-4"><div className="glass-brutalist rounded-[1.6rem] p-5"><p className="text-xl font-bold text-white">{form.fullName}</p><p className="mt-2 text-sm text-white/45">{form.uid} / {form.course} / {form.branch} / {form.year}</p><p className="mt-2 text-sm text-white/45">{form.email} / {form.phone}</p><div className="mt-3 flex flex-wrap gap-2">{links.map(([label, href, Icon]: any) => <a href={href} className="brutalist-btn-dark inline-flex items-center gap-2 rounded-xl px-3 py-2 text-xs" key={label}><Icon size={13} /> {label}</a>)}</div></div><div className="glass-brutalist rounded-[1.6rem] p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-sm text-white/40">Selected path</p><p className="mt-2 text-xl font-bold text-white">{team?.name || "-"} / {role?.name || "-"}</p></div><button type="button" onClick={() => onEdit(2)} className="brutalist-btn-dark rounded-xl px-3 py-2 text-xs">Edit team</button></div></div>{questions.map((question: Question) => <div className="glass-brutalist rounded-[1.6rem] p-5" key={question.id}><p className="text-sm font-bold text-white">{question.label}</p><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-white/45">{answers[question.id] ? (Array.isArray(answers[question.id]) ? answers[question.id].join(", ") : String(answers[question.id])) : "-"}</p></div>)}{files.length ? <div className="glass-brutalist rounded-[1.6rem] p-5"><p className="text-sm font-semibold text-white/72">Uploads</p><div className="mt-3 flex flex-wrap gap-2">{files.map((file: FileAsset) => <a className="brutalist-btn-dark rounded-xl px-3 py-2 text-xs" href={file.url} key={file.url}>{file.label}</a>)}</div></div> : null}</div></div>;
}
    ...
    <div className="grid min-h-[540px] place-items-center text-center px-4">
      <div className="max-w-md w-full">
        ...
        <motion.h2
          ...
          className="text-4xl font-bold tracking-[-.05em] text-white"
        >
        ...
        >
          <Link
            href="/"
            className="brutalist-btn-dark inline-flex min-h-12 w-full sm:w-auto items-center justify-center gap-2 rounded-xl px-6 text-xs text-white"
          >
            <Check size={14} />
