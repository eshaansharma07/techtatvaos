...
import { ArrowRight, ArrowUpRight, ChevronRight, Orbit, Sparkles, Ticket, Users } from "lucide-react";
...
export default async function Home() {
  ...
  return <PublicShell>
    ...
    <section className="hidden md:block border-y border-white/10 bg-black/10"><div className="mx-auto max-w-7xl xl:max-w-[1380px] 2xl:max-w-[1536px] px-5 py-20 md:px-6 md:py-28"><Reveal><div className="flex flex-col md:flex-row md:items-end justify-between mb-12"><SectionTitle eyebrow="ONE SYSTEM / MANY DISCIPLINES" title="Teams building the future." copy="Explore the public team structure and the disciplines behind club work."/><Link className="brutalist-btn-dark inline-flex min-h-12 items-center gap-2 rounded-xl px-5 py-3 text-sm font-bold" href="/teams">Explore the network <ArrowRight size={15}/></Link></div></Reveal><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{teams.length?teams.slice(0,6).map((team,i)=><Reveal key={team.id} delay={i*.04}><div className="glass-brutalist group rounded-[22px] p-6 transition duration-300 hover:-translate-y-1 hover:border-[#00FF66]/50"><div className="flex justify-between"><Orbit size={20} className="text-[#00FF66]"/><span className="brutalist-btn-dark rounded-xl px-3 py-1 text-xs text-white/80 font-bold">{team.members}</span></div><h3 className="mt-8 text-lg font-bold text-white">{team.name}</h3><p className="mt-3 text-sm leading-6 text-white/40">{team.description || "Team details coming soon."}</p></div></Reveal>):<EmptyState title="Team information is coming soon." copy="The public team structure has not been published yet."/>}</div></div></section>
