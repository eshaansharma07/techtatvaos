...
const technicalColors = ["from-amber-500/22 to-purple-400/10", "from-yellow-400/22 to-amber-500/10", "from-rose-500/22 to-red-500/10"];
...
function PersonNode({ label, name, sub, photo, tone = "violet" }: { label: string; name?: string; sub?: string; photo?: string; tone?: "violet" | "purple" | "fuchsia" | "amber" | "rose" }) {
  const tones = {
    violet: "border-purple-500 bg-black/40 text-white shadow-purple-500/10",
    purple: "border-purple-500/60 bg-black/40 text-white shadow-purple-950/25",
    fuchsia: "border-purple-500/55 bg-black/40 text-white shadow-fuchsia-950/30",
    amber: "border-purple-500/40 bg-black/40 text-white shadow-amber-950/25",
    rose: "border-purple-500/20 bg-black/40 text-white shadow-rose-950/25"
  };
...
function AdvisoryRow({ info }: { info: Record<string, any> }) {
  ...
  return (
    <div className={`mx-auto grid w-full max-w-6xl gap-4 ${advisors.length + (coFaculty ? 2 : 1) >= 4 ? "md:grid-cols-4" : advisors.length + (coFaculty ? 2 : 1) >= 3 ? "md:grid-cols-3" : advisors.length + (coFaculty ? 2 : 1) >= 2 ? "md:grid-cols-2" : "md:grid-cols-1"}`}>
      <PersonNode label="Faculty Champion" name={info.facultyChampionName} sub={info.facultyChampionEmail} photo={info.facultyChampionPhoto} tone="purple" />
      {coFaculty ? <PersonNode label="Co-Faculty Champion" name={info.coFacultyChampionName} sub={info.coFacultyChampionEmail || info.coFacultyChampionPhone} photo={info.coFacultyChampionPhoto} tone="purple" /> : null}
  ...
  return (
    <div className="relative rounded-[2rem] glass-brutalist p-4 backdrop-blur-xl">
      <div className="rounded-xl border border-white/10 bg-black/40 p-4 text-center">
        <p className="text-[10px] font-bold uppercase tracking-[.18em] text-purple-400">Team Lead Group</p>
      ...
      <div className="rounded-[1.5rem] border border-white/10 bg-black/40 p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-[10px] uppercase tracking-[.18em] text-purple-400 font-bold">{team.members} active members</p>
          </div>
...
function TeamLane({ title, subtitle, teams, palette, tone }: { title: string; subtitle: string; teams: PublicTeam[]; palette: string[]; tone: "purple" | "fuchsia" }) {
  return (
    <div className="relative">
    ...
    <PublicShell>
      <section className="relative mx-auto max-w-7xl xl:max-w-[1380px] 2xl:max-w-[1536px] px-5 pb-20 pt-32 md:px-6 md:pb-28 md:pt-44 spatial-grid-bg">
        ...
        <div className="team-hero-compact glass-brutalist relative overflow-hidden rounded-[2rem] px-5 py-9 text-center md:rounded-[2.2rem] md:px-10 md:py-10">
          <p className="flex items-center justify-center gap-2 text-[10px] font-bold tracking-[.34em] text-purple-400">
            <Sparkles size={13} />
        ...
        <div className="relative mt-10 hidden overflow-hidden rounded-[2.4rem] glass-brutalist p-5 shadow-2xl md:block md:p-8">
          <div className="relative grid gap-8">
            <div className="glass-brutalist rounded-[2rem] p-6">
              <p className="mb-5 text-center text-[10px] font-bold uppercase tracking-[.24em] text-purple-400">Advisory Tree</p>
              <AdvisoryRow info={info} />
            </div>

            <div className="glass-brutalist rounded-[2rem] p-6">
              <p className="mb-5 text-center text-[10px] font-bold uppercase tracking-[.24em] text-purple-400">Club Operations Tree</p>
              <OperationsRoot info={info} />
              <div className="mx-auto h-12 w-px bg-gradient-to-b from-[rgba(168, 85, 247, 1)]/60 to-white/0" />
              <div className="mx-auto hidden h-px max-w-4xl bg-gradient-to-r from-purple-300/0 via-purple-500/50 to-fuchsia-300/0 md:block" />
