...
export async function GET(req:NextRequest){await connectDB();const status=req.nextUrl.searchParams.get("status");const query=status?{status}:{status:{$in:["published","active","completed"]}};return NextResponse.json(await Event.find(query).sort({startAt:1}).populate("team","name slug").lean())}
