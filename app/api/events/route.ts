...
export async function POST(req:NextRequest){const session=await auth();if(!["super_admin","president","vice_president","secretary","team_lead"].includes((session?.user as {role?:string})?.role||""))return NextResponse.json({error:"Forbidden"},{status:403});const parsed=eventInput.safeParse(await req.json());if(!parsed.success)return NextResponse.json({error:parsed.error.flatten()},{status:400});await connectDB();return NextResponse.json(await Event.create(parsed.data),{status:201})}
