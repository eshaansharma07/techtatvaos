...
const studentFrom = (user: any): ExportStudent | null => {
  if (!user) return null;
  return {
    uid: user.uid || "",
    name: user.name || "",
...
export async function GET(req: NextRequest) {
  ...
  const [presentRows, registrations] = await Promise.all([
    Attendance.find({ event: id, status: "present" }).populate("user", "uid name program semester").sort({ markedAt: 1 }).lean(),
    EventRegistration.find({ event: id, status: "confirmed" })
  ...
  for (const registration of registrations as any[]) {
    const candidates = [registration.user, ...(registration.teamMembers || []).map((member: any) => member.user ? member : member)];
    for (const candidate of candidates) {
      const id = objectId(candidate.user || candidate);
      if (!id || seen.has(id) || !presentIds.has(id)) continue;
      const student = studentFrom(candidate.user ? candidate : candidate);
      if (student?.name) {
  ...
  for (const row of presentRows as any[]) {
    const id = objectId(row.user);
    const student = studentFrom(row.user);
    if (id && !seen.has(id) && student?.name) {
      students.push(student);
      seen.add(id);
    }
  }
