...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    await audit(req, "portal.attendance.present", { entityType: "attendance", entityId: String(record._id), event: body.event, user: body.user });
    const res = NextResponse.json(record);
    res.headers.set("Cache-Control", "no-store, no-cache, must-revalidate");
    return res;
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Could not mark present" }, { status: 400 });
  }
}
