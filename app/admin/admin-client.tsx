...
type Resource = "users" | "teams" | "events" | "tasks" | "announcements" | "sponsors" | "achievements" | "gallery" | "contacts" | "settings";
...
const config: Record<Exclude<Module, "Overview" | "Attendance" | "Settings">, { key: string; resource: Resource; fields: Field[] }> = {
  Members: { key: "users", resource: "users", fields: [["name","Name"],["email","Email","email"],["team","Team","team-select"],["uid","UID"],["registrationNumber","Registration number"],["department","Department"],["program","Program"],["semester","Semester","number"],["phone","Phone"]] },
  Teams: { key: "teams", resource: "teams", fields: [["name","Team name"],["slug","Slug"],["description","Description"],["order","Display order","number"]] },
  Events: { key: "events", resource: "events", fields: [["title","Title"],["slug","Slug"],["description","Description"],["venue","Venue"],["capacity","Capacity","number"],["category","Category"],["team","Team","team-select"],["status","Status"],["registrationOpen","Registration open: true/false"],["startAt","Start date/time","datetime-local"],["endAt","End date/time","datetime-local"],["banner","Banner URL"]] },
  ...
  Media: { key: "gallery", resource: "gallery", fields: [["title","Album title"],["url","Cloudinary/media URL"],["publicId","Cloudinary public id"],["kind","image/video"],["caption","Caption"],["published","Published: true/false"]] }
};
...
const extraFields: Record<Resource, Field[]> = {
  ...
  sponsors: [["name","Sponsor name"],["logo","Logo URL"],["website","Website"],["level","Sponsorship level"],["active","Active: true/false"]],
  achievements: [["title","Title"],["description","Description"],["kind","Kind"],["awardedAt","Awarded at","date"],["image","Image URL"],["featured","Featured: true/false"]],
...
function rawValue(item:any, key:string){const value=item?.[key];if(value && typeof value==="object")return String(value._id || value.id || "");return value ?? ""}
function AdminLogo(){return <a href="/" className="flex items-center gap-3 font-semibold tracking-tight"><span className="grid h-9 w-9 place-items-center rounded-xl border border-violet-400/40 bg-violet-500/10 text-violet-300"><Hexagon size={18}/></span><span>TECH TATVA <i className="font-normal text-white/40">/ OS</i></span></a>}

export function AdminClient({ initialData, userName }: { initialData: Data; userName: string }) {
  ...
  async function submit(formData:FormData){
    ...
    const url=drawer.item?`/api/admin/${drawer.resource}/${idOf(drawer.item)}`:`/api/admin/${drawer.resource}`;
  ...
  async function remove(resource:Resource,item:any){
    ...
    setBusy(false);
    setPanel(res.ok?"Record removed from public/active views.":"Delete/archive failed.");
    await refresh();
  }
  ...
  function rowsFor(module:Module){
    ...
    if(module==="Teams")return (data.teams||[]).map((t:any)=>[t.name,t.slug,t.description||"-",`${t.members?.length||0} members`,t.active===false?"inactive":"active",t]);
  ...
  return <main className="min-h-screen bg-[#08070b] text-white">
    ...
    <div className="p-5 md:p-8"><Header active={active} data={data} open={setDrawer} setPanel={setPanel}/>{active==="Overview"?<Overview counts={counts} chart={chart} setActive={setActive}/>:active==="Attendance"?<Attendance data={data} setPanel={setPanel}/>:active==="Settings"?<Settings info={data.clubInfo||{}} open={setDrawer}/>:<Workspace active={active} rows={filtered} open={setDrawer} remove={remove}/>}<div className="mt-4 rounded-xl border border-violet-300/15 bg-violet-500/[.07] p-5"><p className="text-[10px] tracking-[.2em] text-violet-300">ACTION PANEL</p><p className="mt-3 text-sm leading-6 text-white/65">{panel}</p></div></div></section>
    ...
    {drawer?<div className="fixed inset-0 z-50 bg-black/70 p-4 backdrop-blur-sm"><div className="ml-auto h-full max-w-xl overflow-y-auto rounded-2xl border border-white/10 bg-[#111016] p-6"><div className="flex items-center justify-between"><h2 className="text-xl">{drawer.title}</h2><button onClick={()=>setDrawer(null)} className="text-sm text-white/45">Close</button></div><form action={submit} className="mt-6 grid gap-4">{drawer.fields.map(([name,label,type])=>{const source={...(drawer.defaults||{}),...(drawer.item||{})};return <label className="text-[10px] tracking-wider text-white/35" key={name}>{label.toUpperCase()}{type==="team-select"?<select name={name} defaultValue={rawValue(source,name)} className="mt-2 w-full rounded-lg border border-white/[.07] bg-black/25 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50"><option value="">No team</option>{(data.teams||[]).filter((team:any)=>team.active!==false).map((team:any)=><option value={idOf(team)} key={idOf(team)}>{team.name}</option>)}</select>:name==="description"||name==="body"||name==="aboutCopy"?<textarea name={name} defaultValue={rawValue(source,name)} rows={5} className="mt-2 w-full rounded-lg border border-white/[.07] bg-black/25 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50"/>:<input name={name} type={type||"text"} defaultValue={rawValue(source,name)} className="mt-2 w-full rounded-lg border border-white/[.07] bg-black/25 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50"/>}</label>})}<button disabled={busy} className="rounded-lg bg-white py-3 text-sm font-semibold text-black disabled:opacity-60">{busy?"Saving...":"Save"}</button></form></div></div>:null}
  </main>
}
...
function Workspace({active,rows,open,remove}:{active:Module;rows:any[];open:(drawer:any)=>void;remove:(resource:Resource,item:any)=>void}){const c=config[active as keyof typeof config];return <div className="mt-7 grid gap-4 lg:grid-cols-[1fr_.6fr]"><div className="glass rounded-xl p-5"><p className="text-sm">{active} workspace</p>{rows.length?<div className="mt-4 overflow-hidden rounded-xl border border-white/[.06]">{rows.map((row:any[])=>{const item=row[row.length-1];const resource=(item.__resource||c.resource) as Resource;const fields=extraFields[resource] || c.fields;return <div className="grid gap-3 border-b border-white/[.05] bg-black/15 p-4 text-xs last:border-0 md:grid-cols-[1.1fr_.8fr_.8fr_.8fr_auto_auto]" key={idOf(item)}>{row.slice(0,-1).slice(0,4).map((cell:any,index:number)=><button onClick={()=>open({resource,title:`Edit ${active.slice(0,-1)}`,fields,item})} className={`text-left ${index===0?"text-white/75":"text-white/45"}`} key={`${idOf(item)}-${index}`}>{String(cell)}</button>)}{active==="Teams"?<button onClick={()=>open({resource:"users",title:`Add member to ${item.name}`,fields:config.Members.fields,defaults:{team:idOf(item)}})} className="text-left text-violet-300">Add member</button>:null}<button onClick={()=>remove(resource,item)} className="text-left text-rose-300">Archive</button></div>})}</div>:<p className="mt-5 rounded-xl border border-white/[.06] bg-white/[.025] p-6 text-sm text-white/45">No records yet. Add one from the button above.</p>}</div><div className="glass rounded-xl p-5"><p className="text-sm">Module actions</p>{active==="Media"?(["gallery","sponsors","achievements"] as Resource[]).map((resource)=><button onClick={()=>open({resource,title:`Add ${resource}`,fields:extraFields[resource]})} className="mt-3 block w-full rounded-xl border border-white/[.07] bg-white/[.035] px-4 py-3 text-left text-xs text-white/60 transition hover:bg-violet-500/[.08]" key={resource}>Add {resource}</button>):<button onClick={()=>open({resource:c.resource,title:`Add ${active.slice(0,-1)}`,fields:c.fields})} className="mt-3 block w-full rounded-xl border border-white/[.07] bg-white/[.035] px-4 py-3 text-left text-xs text-white/60 transition hover:bg-violet-500/[.08]">Create record</button>}<p className="mt-3 rounded-xl border border-white/[.07] bg-white/[.035] px-4 py-3 text-xs text-white/55">{active==="Teams"?"Use the Add member action on a team row to preselect that team.":"Archive uses safe public removal."}</p></div></div>}
...
function Settings({info,open}:{info:any;open:(drawer:any)=>void}){const fields:Field[]=[["logo","Logo URL"],["website","Website URL"],["email","Public email"],["location","Location"],["footerCopy","Footer copy"],["aboutTitle","About title"],["aboutCopy","About copy"],["vision","Vision"],["mission","Mission"]];return <div className="mt-7 grid gap-4 lg:grid-cols-[1fr_.7fr]"><div className="glass rounded-xl p-5"><p className="text-sm">Club branding and public information</p>{["logo","website","email","location","aboutTitle","vision","mission"].map((key)=><div className="mt-3 rounded-xl border border-white/[.06] bg-black/15 p-4 text-xs" key={key}><p className="text-white/35">{key}</p><p className="mt-2 text-white/70">{info[key] || "Not set"}</p></div>)}</div><div className="glass rounded-xl p-5"><p className="text-sm">Settings actions</p><button onClick={()=>open({resource:"settings",title:"Update club branding and public info",fields})} className="mt-4 w-full rounded-xl bg-white px-4 py-3 text-xs font-semibold text-black">Update public settings</button><p className="mt-4 text-xs leading-6 text-white/40">These values drive About, Contact, logo, website link, and future branding surfaces. No hardcoded club facts are used.</p></div></div>}
