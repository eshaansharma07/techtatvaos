...
const config: Record<Exclude<Module, "Overview" | "Attendance" | "Settings">, { key: string; resource: Resource; fields: [string, string, string?][] }> = {
  Members: { key: "users", resource: "users", fields: [["name","Name"],["email","Email","email"],["uid","UID"],["registrationNumber","Registration number"],["department","Department"],["program","Program"],["semester","Semester","number"],["phone","Phone"]] },
  Teams: { key: "teams", resource: "teams", fields: [["name","Team name"],["slug","Slug"],["description","Description"],["order","Display order","number"]] },
  Events: { key: "events", resource: "events", fields: [["title","Title"],["slug","Slug"],["description","Description"],["venue","Venue"],["capacity","Capacity","number"],["category","Category"],["status","Status"],["registrationOpen","Registration open: true/false"],["startAt","Start date/time","datetime-local"],["endAt","End date/time","datetime-local"],["banner","Banner URL"]] },
  Tasks: { key: "tasks", resource: "tasks", fields: [["title","Title"],["description","Description"],["dueAt","Due date","datetime-local"],["status","Status"],["priority","Priority"]] },
...
const extraFields: Record<Resource, [string,string,string?][]> = {
...
export function AdminClient({ initialData, userName }: { initialData: Data; userName: string }) {
  ...
  const [drawer,setDrawer]=useState<{resource:Resource; title:string; fields:[string,string,string?][]; item?:any}|null>(null);
  ...
  function rowsFor(module:Module){
    ...
    if(module==="Teams")return (data.teams||[]).map((t:any)=>[t.name,t.slug,t.description||"-",t.active===false?"inactive":"active",String(t.order||""),t]);
  ...
  return <main className="min-h-screen bg-[#08070b] text-white">
    ...
    {drawer?<div className="fixed inset-0 z-50 bg-black/70 p-4 backdrop-blur-sm"><div className="ml-auto h-full max-w-xl overflow-y-auto rounded-2xl border border-white/10 bg-[#111016] p-6"><div className="flex items-center justify-between"><h2 className="text-xl">{drawer.title}</h2><button onClick={()=>setDrawer(null)} className="text-sm text-white/45">Close</button></div><form action={submit} className="mt-6 grid gap-4">{drawer.fields.map(([name,label,type])=><label className="text-[10px] tracking-wider text-white/35" key={name}>{label.toUpperCase()}{name==="description"||name==="body"?<textarea name={name} defaultValue={valueOf(drawer.item||{},name)} rows={5} className="mt-2 w-full rounded-lg border border-white/[.07] bg-black/25 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50"/>:<input name={name} type={type||"text"} defaultValue={valueOf(drawer.item||{},name)} className="mt-2 w-full rounded-lg border border-white/[.07] bg-black/25 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50"/>}</label>)}<button disabled={busy} className="rounded-lg bg-white py-3 text-sm font-semibold text-black disabled:opacity-60">{busy?"Saving...":"Save"}</button></form></div></div>:null}
  </main>
}
...
function Workspace({active,rows,open,remove}:{active:Module;rows:any[];open:(drawer:any)=>void;remove:(resource:Resource,item:any)=>void}){const c=config[active as keyof typeof config];return <div className="mt-7 grid gap-4 lg:grid-cols-[1fr_.6fr]"><div className="glass rounded-xl p-5"><p className="text-sm">{active} workspace</p>{rows.length?<div className="mt-4 overflow-hidden rounded-xl border border-white/[.06]">{rows.map((row:any[])=>{const item=row[row.length-1];const resource=(item.__resource||c.resource) as Resource;const fields=extraFields[resource] || c.fields;return <div className="grid gap-3 border-b border-white/[.05] bg-black/15 p-4 text-xs last:border-0 md:grid-cols-[1.1fr_.8fr_.8fr_.8fr_auto]" key={idOf(item)}>{row.slice(0,-1).slice(0,4).map((cell:any,index:number)=><button onClick={()=>open({resource,title:`Edit ${active.slice(0,-1)}`,fields,item})} className={`text-left ${index===0?"text-white/75":"text-white/45"}`} key={`${idOf(item)}-${index}`}>{String(cell)}</button>)}<button onClick={()=>remove(resource,item)} className="text-left text-rose-300">Archive</button></div>})}</div>:<p className="mt-5 rounded-xl border border-white/[.06] bg-white/[.025] p-6 text-sm text-white/45">No records yet. Add one from the button above.</p>}</div><div className="glass rounded-xl p-5"><p className="text-sm">Module actions</p>{active==="Media"?(["gallery","sponsors","achievements"] as Resource[]).map((resource)=><button onClick={()=>open({resource,title:`Add ${resource}`,fields:extraFields[resource]})} className="mt-3 block w-full rounded-xl border border-white/[.07] bg-white/[.035] px-4 py-3 text-left text-xs text-white/60 transition hover:bg-violet-500/[.08]" key={resource}>Add {resource}</button>):<button onClick={()=>open({resource:c.resource,title:`Add ${active.slice(0,-1)}`,fields:c.fields})} className="mt-3 block w-full rounded-xl border border-white/[.07] bg-white/[.035] px-4 py-3 text-left text-xs text-white/60 transition hover:bg-violet-500/[.08]">Create record</button>}<p className="mt-3 rounded-xl border border-white/[.07] bg-white/[.035] px-4 py-3 text-xs text-white/55">Archive uses safe public removal.</p></div></div>}
