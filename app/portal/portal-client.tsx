...
const config: Record<Exclude<Module, "Overview" | "Attendance" | "Settings">, { key: string; resource: Resource; fields: Field[] }> = {
  ...
  Teams: { key: "teams", resource: "teams", fields: [["name","Team name"],["slug","Slug"],["description","Description"],["lead","Team lead","member-select"],["coLeads","Co-leads","member-multi-select"],["order","Display order","number"],["active","Active: true/false"]] },
  Events: { key: "events", resource: "events", fields: [["title","Title"],["slug","Slug"],["description","Description"],["venue","Venue"],["capacity","Capacity","number"],["category","Category"],["team","Team","team-select"],["leads","Event leads","member-multi-select"],["participationMode","Participation type","participation-select"],["maxTeamSize","Maximum team size","number"],["status","Public status","status-select"],["registrationOpen","Registration open","boolean-select"],["registrationStart","Registration start","datetime-local"],["registrationEnd","Registration end","datetime-local"],["startAt","Event start date/time","datetime-local"],["endAt","Event end date/time","datetime-local"],["banner","Event banner","upload:image"]] },
...
function Header({active,data,open,setPanel}:{active:Module;data:Data;open:(drawer:any)=>void;setPanel:(text:string)=>void}){
  ...
  return <div className="portal-hero flex flex-wrap items-center justify-between gap-5 rounded-[1.75rem] p-6 md:p-8"><div><p className="text-[10px] font-semibold tracking-[.28em] text-violet-200/75">COMMAND CENTER / {active.toUpperCase()}</p><h2 className="mt-3 text-4xl font-semibold tracking-[-.055em] md:text-5xl">{active==="Overview"?"Club intelligence":active}</h2><p className="mt-3 max-w-2xl text-sm leading-6 text-white/48">Live operational controls for members, teams, events, attendance, media, and public club content.</p></div><button type="button" onClick={()=>{if(active==="Overview"){exportDashboardSummary(data);setPanel("Dashboard summary CSV downloaded.");}else if(active==="Attendance"){window.dispatchEvent(new Event("portal-download-attendance"));setPanel("Generating attendance sheet for the selected event...");}else if(active==="Settings")open({resource:"settings",title:"Update club branding, faculty, and office bearers",fields:settingsFields});else{const c=config[active as keyof typeof config];open({resource:c.resource,title:`Add ${active.slice(0,-1)}`,fields:c.fields,defaults:active==="Events"?{status:"published",registrationOpen:"true"}:{}})}}} className="portal-command-button flex items-center gap-2 self-end rounded-2xl px-5 py-3 text-xs font-semibold transition hover:-translate-y-0.5">{active==="Overview"||active==="Attendance"?<Download size={14}/>:<Plus size={14}/>}<span>{action}</span></button></div>
}
...
const creativeTeamWords = ["design","media","creative","content","marketing","social","outreach","sponsor","photo","video"];
function splitPortalTeams(teams:any[]){const creative:any[]=[];const operations:any[]=[];(teams||[]).filter((team:any)=>team.active!==false).forEach((team:any)=>{const text=`${team.name||""} ${team.description||""}`.toLowerCase();(creativeTeamWords.some((word)=>text.includes(word))?creative:operations).push(team)});return {operations,creative}}
...
function TeamStructureEditor({data,open,remove,restore}:{data:Data;open:(drawer:any)=>void;remove:(resource:Resource,item:any)=>void;restore:(resource:Resource,item:any)=>void}){const info=data.clubInfo||{};const {operations,creative}=splitPortalTeams(data.teams||[]);return <div className="mt-7 grid gap-4 xl:grid-cols-[1fr_.32fr]"><div className="relative overflow-hidden rounded-[2rem] border border-white/[.08] bg-[#05070d]/75 p-5 md:p-7"><div className="absolute inset-0 grid-bg opacity-20"/><div className="relative"><PortalStructureNode label="1. Faculty Champion" name={info.facultyChampionName} meta={info.facultyChampionEmail || "Update from Settings"} tone="emerald" onEdit={()=>open({resource:"settings",title:"Update faculty champion and office bearers",fields:settingsFields})}/><div className="mx-auto h-8 w-px bg-white/25"/><PortalStructureNode label="2. Core Operations" name={info.secretaryName} meta="Secretary controls club operations and team structure" tone="violet" onEdit={()=>open({resource:"settings",title:"Update secretary and joint secretaries",fields:settingsFields})}/><div className="mx-auto h-10 w-px bg-white/25"/><div className="mx-auto hidden h-px max-w-4xl bg-gradient-to-r from-cyan-300/0 via-cyan-300/45 to-fuchsia-300/45 md:block"/><div className="mt-6 grid gap-8 2xl:grid-cols-2"><TeamLaneEditor title="3. Joint Secretary (Technical & Operations)" subtitle={info.jointSecretaryOneName || "Assign in Settings"} teams={operations} tone="cyan" open={open} remove={remove} restore={restore}/><TeamLaneEditor title="4. Joint Secretary (Media & Creative)" subtitle={info.jointSecretaryTwoName || "Assign in Settings"} teams={creative} tone="fuchsia" open={open} remove={remove} restore={restore}/></div></div></div><div className="glass rounded-[1.5rem] p-5"><p className="text-sm font-semibold">Structure actions</p><button onClick={()=>open({resource:"teams",title:"Create team",fields:config.Teams.fields,defaults:{active:"true"}})} className="portal-command-button mt-4 w-full rounded-2xl px-4 py-3 text-xs font-semibold">Create team</button><button onClick={()=>open({resource:"settings",title:"Update faculty, secretary, and joint secretaries",fields:settingsFields})} className="portal-mini-button mt-3 w-full rounded-2xl px-4 py-3 text-xs font-semibold text-violet-100">Edit top hierarchy</button><p className="mt-4 text-xs leading-6 text-white/42">Faculty Champion sits at the top. Core Operations comes under the Faculty Champion. Teams are split automatically: Design/Media/Creative/Outreach go to the creative lane; all other teams go to technical and operations.</p></div></div>}

function Workspace({active,rows,open,remove,restore,patch,duplicateEvent}:{active:Module;rows:any[];open:(drawer:any)=>void;remove:(resource:Resource,item:any)=>void;restore:(resource:Resource,item:any)=>void;patch:(resource:Resource,item:any,body:Record<string, any>,message:string)=>void;duplicateEvent:(item:any)=>void}){const c=config[active as keyof typeof config];return <div className="mt-7 grid gap-4 lg:grid-cols-[1fr_.6fr]"><div className="glass rounded-xl p-5"><p className="text-sm">{active} workspace</p>{rows.length?<div className="mt-4 overflow-hidden rounded-xl border border-white/[.06]">{rows.map((row:any[])=>{const item=row[row.length-1];const resource=(item.__resource||c.resource) as Resource;const fields=extraFields[resource] || c.fields;const inactive=item.active===false || item.published===false || item.status==="archived" || item.status==="inactive";const isEvent=active==="Events";return <div className="grid gap-3 border-b border-white/[.05] bg-black/15 p-4 text-xs last:border-0 md:grid-cols-[1.1fr_.8fr_.8fr_.8fr_auto]" key={idOf(item)}>{row.slice(0,-1).slice(0,4).map((cell:any,index:number)=><button onClick={()=>open({resource,title:`Edit ${active.slice(0,-1)}`,fields,item})} className={`text-left ${index===0?"text-white/75":"text-white/45"}`} key={`${idOf(item)}-${index}`}>{String(cell)}</button>)}<div className="flex flex-wrap gap-3">{active==="Teams"&&item.active!==false?<button onClick={()=>open({resource:"users",title:`Add member to ${item.name}`,fields:config.Members.fields,defaults:{team:idOf(item)}})} className="portal-link-action text-violet-200">Add member</button>:active==="Teams"?<span className="text-white/25">Inactive</span>:null}{isEvent?<><button onClick={()=>patch(resource,item,{status:"published"},"Event published. It is visible on the public website.")} className="portal-link-action text-emerald-200">Publish</button><button onClick={()=>patch(resource,item,{status:"draft",registrationOpen:"false"},"Event moved to draft and hidden publicly.")} className="portal-link-action text-white/55">Draft</button><button onClick={()=>patch(resource,item,{registrationOpen:String(!item.registrationOpen),status:item.status==="draft"?"published":item.status},"Registration setting updated.")} className="portal-link-action text-violet-200">{item.registrationOpen?"Close reg":"Open reg"}</button><button onClick={()=>duplicateEvent(item)} className="portal-link-action text-sky-200">Duplicate</button></>:null}{inactive?<button onClick={()=>restore(resource,item)} className="portal-link-action text-emerald-200">Restore</button>:!isEvent?<button onClick={()=>remove(resource,item)} className="portal-link-action text-rose-200">{active==="Members"?"Remove":"Archive"}</button>:null}{isEvent?<button onClick={()=>remove(resource,item)} className="portal-link-action text-rose-200">Delete</button>:null}</div></div>})}</div>:<p className="mt-5 rounded-xl border border-white/[.06] bg-white/[.025] p-6 text-sm text-white/45">No records yet. Add one from the button above.</p>}</div><div className="glass rounded-xl p-5"><p className="text-sm">Module actions</p>{active==="Media"?(["gallery","sponsors","achievements"] as Resource[]).map((resource)=><button onClick={()=>open({resource,title:`Add ${resource}`,fields:extraFields[resource]})} className="portal-mini-button mt-3 block w-full rounded-2xl px-4 py-3 text-left text-xs text-white/68 transition hover:-translate-y-0.5 hover:text-white" key={resource}>Add {resource}</button>):<button onClick={()=>open({resource:c.resource,title:`Add ${active.slice(0,-1)}`,fields:c.fields,defaults:active==="Events"?{status:"published",registrationOpen:"true"}:{}})} className="portal-mini-button mt-3 block w-full rounded-2xl px-4 py-3 text-left text-xs text-white/68 transition hover:-translate-y-0.5 hover:text-white">Create record</button>}<p className="portal-mini-button mt-3 rounded-2xl px-4 py-3 text-xs leading-5 text-white/60">{active==="Teams"?"Lead and co-leads are saved per team. Club faculty and secretary details live in Settings.":active==="Events"?"Published or active events appear publicly. Draft and archived events stay hidden. Delete removes the event plus its registrations and attendance.":active==="Members"?"Removed members are made inactive so their data is preserved and can be restored.":"Archive uses safe public removal."}</p></div></div>}
  ...
  return (
    <div className="mt-7 grid gap-4 xl:grid-cols-[1fr_.42fr]">
      ...
      <div className="glass rounded-xl p-5">
        ...
        {selected ? <div className="mt-4 grid gap-3 border-t border-white/[.06] pt-4"><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-white/35">Certificates</p><a download className="portal-command-button rounded-2xl px-4 py-3 text-center text-xs font-semibold" href={`/api/certificates/export?event=${selected}&type=participation`}>Download participation certificates</a><a download className="portal-mini-button rounded-2xl px-4 py-3 text-center text-xs text-white/70" href={`/api/certificates/export?event=${selected}&type=winner`}>Download winner certificates</a></div> : null}
