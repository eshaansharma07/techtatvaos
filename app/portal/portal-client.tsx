...
function PortalStructureNode({label,name,meta,tone="violet",onEdit}:{label:string;name?:string;meta?:string;tone?:"violet"|"emerald"|"fuchsia";onEdit?:()=>void}){const tones={violet:"border-violet-300/25 bg-violet-500/10 text-violet-100",emerald:"border-emerald-300/25 bg-emerald-500/10 text-emerald-100",fuchsia:"border-fuchsia-300/25 bg-fuchsia-500/10 text-fuchsia-100"};return <button type="button" onClick={onEdit} className={`mx-auto block w-full max-w-lg rounded-3xl border p-5 text-center transition hover:-translate-y-0.5 ${tones[tone]}`}><p className="text-[10px] font-semibold uppercase tracking-[.2em] text-white/45">{label}</p><p className="mt-2 text-xl font-semibold text-white">{name || "Add details"}</p>{meta?<p className="mt-2 text-xs leading-5 text-white/50">{meta}</p>:null}</button>}
function PortalTeamTreeCard({team,index,open,remove,restore}:{team:any;index:number;open:(drawer:any)=>void;remove:(resource:Resource,item:any)=>void;restore:(resource:Resource,item:any)=>void}){const fields=extraFields.teams;const inactive=team.active===false;const lead=valueOf(team,"lead") || "No lead assigned";const coLeads=asArray(team.coLeads).map((lead:any)=>lead?.name || (typeof lead==="string"?lead:"")).filter(Boolean);return <div className={`rounded-[1.5rem] border p-4 ${inactive?"border-white/[.06] bg-white/[.018] opacity-60":"border-white/[.08] bg-white/[.035]"}`}><div className="rounded-2xl border border-white/[.08] bg-black/25 p-3 text-center"><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-white/35">Team Lead Group</p><p className="mt-2 text-sm font-semibold text-white">{lead}</p></div><div className="mx-auto h-6 w-px bg-white/18"/><div className="rounded-2xl border border-white/[.08] bg-black/25 p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-base font-semibold text-white">{team.name}</p><p className="mt-1 text-[10px] uppercase tracking-[.16em] text-white/35">Order {team.order ?? index + 1} / {asArray(team.members).length} assigned</p></div><Workflow size={17} className="text-violet-200"/></div>{team.description?<p className="mt-3 text-xs leading-5 text-white/45">{team.description}</p>:null}<div className="mt-3 grid gap-2">{coLeads.length?<p className="rounded-xl border border-white/[.07] bg-white/[.035] px-3 py-2 text-xs text-white/65">Co-leads: {coLeads.join(", ")}</p>:<p className="rounded-xl border border-white/[.07] bg-white/[.025] px-3 py-2 text-xs text-white/35">No co-leads assigned</p>}</div><div className="mt-4 flex flex-wrap gap-2"><button onClick={()=>open({resource:"teams",title:`Edit ${team.name}`,fields,item:team})} className="portal-link-action text-violet-200">Edit team</button><button onClick={()=>open({resource:"users",title:`Add member to ${team.name}`,fields:config.Members.fields,defaults:{teams:[idOf(team)]}})} className="portal-link-action text-emerald-200">Add member</button>{inactive?<button onClick={()=>restore("teams",team)} className="portal-link-action text-emerald-200">Restore</button>:<button onClick={()=>remove("teams",team)} className="portal-link-action text-rose-200">Archive</button>}</div></div></div>}
function TeamLaneEditor({title,subtitle,teams,tone,open,remove,restore}:{title:string;subtitle:string;teams:any[];tone:"emerald"|"fuchsia";open:(drawer:any)=>void;remove:(resource:Resource,item:any)=>void;restore:(resource:Resource,item:any)=>void}){return <div><PortalStructureNode label={title} name={subtitle} tone={tone}/><div className="mx-auto h-8 w-px bg-white/20"/><div className="grid gap-4 lg:grid-cols-2">{teams.length?teams.map((team:any,index:number)=><PortalTeamTreeCard key={idOf(team)} team={team} index={index} open={open} remove={remove} restore={restore}/>):<div className="rounded-3xl border border-white/[.08] bg-white/[.025] p-6 text-center text-sm text-white/42 lg:col-span-2">No teams in this lane yet. Create a team and assign its lead/members.</div>}</div></div>}
function PortalAdvisoryRow({info,open}:{info:any;open:(drawer:any)=>void}){const advisors=[info.studentAdvisorOneName,info.studentAdvisorTwoName].filter(Boolean);const coFaculty=info.coFacultyChampionName || info.coFacultyChampionEmail || info.coFacultyChampionPhone;const columns=advisors.length + 1 + (coFaculty?1:0);return <div className={`mx-auto grid w-full max-w-6xl gap-3 ${columns>=4?"md:grid-cols-4":columns>=3?"md:grid-cols-3":columns>=2?"md:grid-cols-2":"md:grid-cols-1"}`}><PortalStructureNode label="Faculty Champion" name={info.facultyChampionName} meta={info.facultyChampionEmail || "Update from Settings"} tone="emerald" onEdit={()=>open({resource:"settings",title:"Update faculty champion and student advisors",fields:settingsFields})}/>{coFaculty?<PortalStructureNode label="Co-Faculty Champion" name={info.coFacultyChampionName} meta={info.coFacultyChampionEmail || info.coFacultyChampionPhone || "Update from Settings"} tone="emerald" onEdit={()=>open({resource:"settings",title:"Update co-faculty champion",fields:settingsFields})}/>:null}{advisors.map((name:string,index:number)=><PortalStructureNode key={`${name}-${index}`} label={`Student Advisor ${index+1}`} name={name} meta={index===0?info.studentAdvisorOneEmail:info.studentAdvisorTwoEmail} tone="emerald" onEdit={()=>open({resource:"settings",title:"Update student advisors",fields:settingsFields})}/>)}</div>}
              ...
              return (
                <div className="grid gap-3 border-b border-white/[.05] bg-black/15 p-4 text-sm last:border-0 md:grid-cols-[1.1fr_.8fr_.8fr_.8fr_auto] md:text-xs" key={idOf(item)}>
                  {row.slice(0, -1).slice(0, 4).map((cell: any, index: number) => (
                    <button onClick={() => open({ resource, title: `Edit ${active === "Hall of Fame" ? "Hall entry" : active.slice(0, -1)}`, fields, item })} className={`rounded-2xl border border-white/[.06] bg-white/[.025] p-3 text-left md:border-0 md:bg-transparent md:p-0 ${index === 0 ? "text-white/80" : "text-white/48"}`} key={`${idOf(item)}-${index}`}>
                      <span className="mb-1 block text-[9px] uppercase tracking-[.18em] text-white/28 md:hidden">{index === 0 ? (active === "Hall of Fame" ? "Hall entry" : active.slice(0, -1)) : `Detail ${index}`}</span>
                      <span>{String(cell)}</span>
                    </button>
                  ))}
                  <div className="flex flex-wrap gap-2 md:gap-3">
                    {active === "Teams" && item.active !== false ? <button onClick={() => open({ resource: "users", title: `Add member to ${item.name}`, fields: config.Members.fields, defaults: { teams: [idOf(item)] } })} className="portal-link-action text-violet-200">Add member</button> : null}
                    {isEvent ? (
                      <>
                        ...
                        <a className="portal-link-action text-amber-200" href={`/api/ai/event-report?event=${idOf(item)}&format=docx`}>Report DOCX</a>
                        <button onClick={() => duplicateEvent(item)} className="portal-link-action text-amber-200">Duplicate</button>
                      </>
                    ) : null}
                    {isMeeting ? (
                      <>
                        <a className="portal-link-action text-fuchsia-200" href={`/api/ai/mom?meeting=${idOf(item)}&format=pdf`}>MOM PDF</a>
                        <a className="portal-link-action text-amber-200" href={`/api/ai/mom?meeting=${idOf(item)}&format=docx`}>MOM DOCX</a>
                      </>
                    ) : null}
                    {active === "Members" ? <button onClick={() => remove(resource, item)} className="portal-link-action text-rose-200">Delete permanently</button> : active === "Contact Messages" ? <button onClick={() => patch(resource, item, { status: item.status === "resolved" ? "new" : "resolved" }, item.status === "resolved" ? "Message reopened." : "Message marked resolved.")} className="portal-link-action text-emerald-200">{item.status === "resolved" ? "Reopen" : "Resolve"}</button> : inactive ? <button onClick={() => restore(resource, item)} className="portal-link-action text-emerald-200">Restore</button> : !isEvent ? <button onClick={() => remove(resource, item)} className="portal-link-action text-rose-200">Archive</button> : null}
...
function RecruitmentDesk({data,open,patch,remove,refresh,setPanel}:{data:Data;open:(drawer:any)=>void;patch:(resource:Resource,item:any,body:Record<string, any>,message:string)=>void;remove:(resource:Resource,item:any)=>void;refresh:()=>Promise<void>;setPanel:(value:string)=>void}) {
  ...
  const filtered = useMemo(()=>applications.filter((item:any)=>{
    ...
    return true;
  }).sort((a:any,b:any)=>String(b.submittedAt||b.createdAt).localeCompare(String(a.submittedAt||a.createdAt))),[applications,query,statusFilter,teamFilter]);
  const pages = Math.max(1, Math.ceil(filtered.length / pageSize));
  const visible = filtered.slice(page * pageSize, page * pageSize + pageSize);
  const exportHref = `/api/recruitment/export?${new URLSearchParams(Object.entries({team:teamFilter!=="all"?teamFilter:"",status:statusFilter!=="all"?statusFilter:""}).filter(([,v])=>v)).toString()}`;
  ...
  return <div className="mt-7 grid gap-4 xl:grid-cols-[1fr_.36fr]">
    <div className="grid gap-4">
      ...
      <div className="glass rounded-xl p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto_auto]"><label className="portal-search flex items-center gap-3 rounded-2xl px-4 py-3 text-white/40"><Search size={14}/><input value={query} onChange={(e)=>{setQuery(e.target.value);setPage(0);}} placeholder="Search name, email, UID..." className="w-full bg-transparent text-sm text-white outline-none placeholder:text-white/25"/></label><select value={statusFilter} onChange={(e)=>{setStatusFilter(e.target.value);setPage(0);}} className="rounded-2xl border border-white/[.08] bg-black/20 px-4 py-3 text-xs text-white/70"><option value="all">All statuses</option><option value="pending">Pending</option><option value="shortlisted">Shortlisted</option><option value="accepted">Accepted</option><option value="rejected">Rejected</option><option value="on_hold">On hold</option></select><select value={teamFilter} onChange={(e)=>{setTeamFilter(e.target.value);setPage(0);}} className="rounded-2xl border border-white/[.08] bg-black/20 px-4 py-3 text-xs text-white/70"><option value="all">All teams</option>{teams.map((team:any)=><option key={idOf(team)} value={idOf(team)}>{team.name}</option>)}</select></div>
        ...
        <div className="mt-4 max-h-[540px] overflow-auto rounded-xl border border-white/[.06]">
          {visible.map((item:any)=><div className="grid gap-3 border-b border-white/[.05] bg-black/15 p-4 text-sm last:border-0 md:grid-cols-[auto_1fr_.8fr_.7fr_auto]" key={idOf(item)}>
            <input type="checkbox" checked={selected.includes(idOf(item))} onChange={()=>toggle(idOf(item))} className="mt-1"/>
            <button onClick={()=>setProfile(item)} className="text-left"><p className="font-semibold text-white/80">{item.fullName}</p><p className="mt-1 text-xs text-white/38">{item.email} / {item.uid}</p></button>
            <p className="text-white/52">{valueOf(item,"team") || "-"}<span className="block text-xs text-white/32">{valueOf(item,"role") || "-"}</span></p>
            <span className={`h-fit rounded-full px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[.14em] ${item.status==="accepted"?"bg-emerald-400/10 text-emerald-200":item.status==="rejected"?"bg-rose-400/10 text-rose-200":item.status==="shortlisted"?"bg-violet-400/10 text-violet-100":item.status==="on_hold"?"bg-amber-400/10 text-amber-100":"bg-white/[.06] text-white/45"}`}>{String(item.status || "pending").replace("_"," ")}</span>
            <div className="flex flex-wrap gap-2"><button onClick={()=>patch("recruitmentApplications",item,{status:"shortlisted"},"Application shortlisted.")} className="portal-link-action text-violet-200">Shortlist</button><button onClick={()=>patch("recruitmentApplications",item,{status:"accepted"},"Application accepted.")} className="portal-link-action text-emerald-200">Accept</button><button onClick={()=>patch("recruitmentApplications",item,{status:"on_hold"},"Application moved on hold.")} className="portal-link-action text-amber-200">Hold</button><button onClick={()=>patch("recruitmentApplications",item,{status:"rejected"},"Application rejected.")} className="portal-link-action text-rose-200">Reject</button></div>
          </div>)}
    ...
    {profile?<div className="fixed inset-0 z-50 bg-black/70 p-4 backdrop-blur-sm"><div className="mx-auto h-full max-w-3xl overflow-y-auto rounded-3xl border border-white/10 bg-[#111016] p-6 shadow-2xl"><div className="flex items-center justify-between gap-3"><div><h2 className="text-2xl font-semibold">{profile.fullName}</h2><p className="mt-1 text-sm text-white/45">{profile.email} / {profile.uid}</p></div><button onClick={()=>setProfile(null)} className="portal-mini-button rounded-full px-3 py-1.5 text-xs text-white/55">Close</button></div><div className="mt-6 grid gap-4 md:grid-cols-2"><div className="rounded-2xl border border-white/[.06] bg-white/[.025] p-4 text-sm"><p className="text-white/35">Personal</p><p className="mt-2">{profile.course} / {profile.branch} / {profile.year}</p><p className="mt-2">{profile.phone}</p></div><div className="rounded-2xl border border-white/[.06] bg-white/[.025] p-4 text-sm"><p className="text-white/35">Path</p><p className="mt-2">{valueOf(profile,"team")} / {valueOf(profile,"role")}</p><p className="mt-2 capitalize">{String(profile.status || "pending").replace("_"," ")}</p></div></div><div className="mt-4 rounded-2xl border border-white/[.06] bg-white/[.025] p-4 text-sm"><p className="text-white/35">Answers</p><div className="mt-3 grid gap-3">{(profile.answers||[]).map((answer:any)=><div key={answer.label}><p className="text-white/70">{answer.label}</p><p className="mt-1 text-white/45">{Array.isArray(answer.value)?answer.value.join(", "):String(answer.value || "-")}</p></div>)}</div></div>{(profile.files||[]).length?<div className="mt-4 rounded-2xl border border-white/[.06] bg-white/[.025] p-4 text-sm"><p className="text-white/35">Uploads</p><div className="mt-3 flex flex-wrap gap-2">{profile.files.map((file:any)=><a className="portal-link-action text-fuchsia-200" href={file.url} key={file.url}>{file.label}</a>)}</div></div>:null}<div className="mt-4 rounded-2xl border border-white/[.06] bg-white/[.025] p-4 text-sm"><p className="text-white/35">Timeline</p><div className="mt-3 grid gap-2">{(profile.timeline||[]).slice().reverse().map((entry:any,index:number)=><p key={index} className="text-xs text-white/45">{entry.at?new Date(entry.at).toLocaleString("en-IN"):""} · {entry.action}{entry.note?` — ${entry.note}`:""}</p>)}</div></div><div className="mt-6 flex flex-wrap gap-2"><button onClick={()=>{open({resource:"recruitmentApplications",title:`Review ${profile.fullName}`,fields:extraFields.recruitmentApplications,item:profile});setProfile(null);}} className="portal-command-button rounded-2xl px-4 py-3 text-xs font-semibold">Edit notes / status</button><button onClick={()=>remove("recruitmentApplications",profile)} className="portal-mini-button rounded-2xl px-4 py-3 text-xs text-rose-200">Archive</button></div></div></div>:null}
  </div>
}
