...
const config: Record<Exclude<Module, "Overview" | "Attendance" | "Settings">, { key: string; resource: Resource; fields: Field[] }> = {
  ...
  Events: { key: "events", resource: "events", fields: [["title","Title"],["slug","Slug"],["description","Description"],["venue","Venue"],["capacity","Capacity","number"],["category","Category"],["team","Team","team-select"],["status","Status"],["registrationOpen","Registration open: true/false"],["startAt","Start date/time","datetime-local"],["endAt","End date/time","datetime-local"],["banner","Event banner","upload:image"]] },
...
function rawValue(item:any, key:string): string | number | string[] {const value=item?.[key];if(Array.isArray(value))return value.map((entry)=>String(rawValue({entry},"entry")));if(value && typeof value==="object")return String(value._id || value.id || "");return value ?? ""}
...
export function PortalClient({ initialData, userName }: { initialData: Data; userName: string }) {
  ...
  async function restore(resource:Resource,item:any){
    setBusy(true);
    const res=await fetch(`/api/admin/${resource}/${idOf(item)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({active:"true",published:"true"})});
    setBusy(false);
  ...
  return <main className="min-h-screen bg-[#08070b] text-white">
    ...
    {drawer?<div className="fixed inset-0 z-50 bg-black/70 p-4 backdrop-blur-sm"><div className="ml-auto h-full max-w-xl overflow-y-auto rounded-2xl border border-white/10 bg-[#111016] p-6"><div className="flex items-center justify-between"><h2 className="text-xl">{drawer.title}</h2><button onClick={()=>setDrawer(null)} className="text-sm text-white/45">Close</button></div><form action={submit} className="mt-6 grid gap-4">{drawer.fields.map(([name,label,type])=>{const source={...(drawer.defaults||{}),...(drawer.item||{})};const selected=rawValue(source,name);return <label className="text-[10px] tracking-wider text-white/35" key={name}>{label.toUpperCase()}{type==="team-select"?<select name={name} defaultValue={selected} className="mt-2 w-full rounded-lg border border-white/[.07] bg-black/25 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50"><option value="">No team</option>{(data.teams||[]).filter((team:any)=>team.active!==false).map((team:any)=><option value={idOf(team)} key={idOf(team)}>{team.name}</option>)}</select>:type==="member-select"?<select name={name} defaultValue={selected} className="mt-2 w-full rounded-lg border border-white/[.07] bg-black/25 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50"><option value="">No member selected</option>{(data.users||[]).filter((user:any)=>user.status!=="inactive").map((user:any)=><option value={idOf(user)} key={idOf(user)}>{memberLabel(user)}</option>)}</select>:type==="member-multi-select"?<select name={name} multiple defaultValue={Array.isArray(selected)?selected:[]} className="mt-2 min-h-36 w-full rounded-lg border border-white/[.07] bg-black/25 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50">{(data.users||[]).filter((user:any)=>user.status!=="inactive").map((user:any)=><option value={idOf(user)} key={idOf(user)}>{memberLabel(user)}</option>)}</select>:type?.startsWith("upload")?<UploadControl name={name} current={String(selected||"")} upload={uploads[name]} uploading={Boolean(uploading[name])} onUpload={async(file)=>{setUploading((state)=>({...state,[name]:true}));setPanel(`Uploading ${file.name}...`);const form=new FormData();form.append("file",file);form.append("folder",`tech-tatva-os/${drawer.resource}`);const res=await fetch("/api/portal/upload",{method:"POST",body:form});const result=await res.json();setUploading((state)=>({...state,[name]:false}));if(!res.ok){setPanel(result.error || "Upload failed");return}setUploads((state)=>({...state,[name]:result}));setPanel(`Uploaded ${file.name}. Now save the form.`)}}/>:name==="description"||name==="body"||name==="aboutCopy"?<textarea name={name} defaultValue={selected} rows={5} className="mt-2 w-full rounded-lg border border-white/[.07] bg-black/25 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50"/>:<input name={name} type={type||"text"} defaultValue={selected} className="mt-2 w-full rounded-lg border border-white/[.07] bg-black/25 px-3 py-3 text-sm text-white outline-none focus:border-violet-400/50"/>}</label>})}<p className="text-[10px] leading-4 text-white/35">Tip: hold Command/Ctrl to select multiple co-leads.</p><button disabled={busy||Object.values(uploading).some(Boolean)} className="rounded-lg bg-white py-3 text-sm font-semibold text-black disabled:opacity-60">{busy?"Saving...":Object.values(uploading).some(Boolean)?"Uploading...":"Save"}</button></form></div></div>:null}
  </main>
}
...
function Workspace({active,rows,open,remove,restore}:{active:Module;rows:any[];open:(drawer:any)=>void;remove:(resource:Resource,item:any)=>void;restore:(resource:Resource,item:any)=>void}){const c=config[active as keyof typeof config];return <div className="mt-7 grid gap-4 lg:grid-cols-[1fr_.6fr]"><div className="glass rounded-xl p-5"><p className="text-sm">{active} workspace</p>{rows.length?<div className="mt-4 overflow-hidden rounded-xl border border-white/[.06]">{rows.map((row:any[])=>{const item=row[row.length-1];const resource=(item.__resource||c.resource) as Resource;const fields=extraFields[resource] || c.fields;const inactive=item.active===false || item.published===false;return <div className="grid gap-3 border-b border-white/[.05] bg-black/15 p-4 text-xs last:border-0 md:grid-cols-[1.1fr_.8fr_.8fr_.8fr_auto_auto]" key={idOf(item)}>{row.slice(0,-1).slice(0,4).map((cell:any,index:number)=><button onClick={()=>open({resource,title:`Edit ${active.slice(0,-1)}`,fields,item})} className={`text-left ${index===0?"text-white/75":"text-white/45"}`} key={`${idOf(item)}-${index}`}>{String(cell)}</button>)}{active==="Teams"&&item.active!==false?<button onClick={()=>open({resource:"users",title:`Add member to ${item.name}`,fields:config.Members.fields,defaults:{team:idOf(item)}})} className="text-left text-violet-300">Add member</button>:active==="Teams"?<span className="text-left text-white/25">Inactive</span>:null}{inactive?<button onClick={()=>restore(resource,item)} className="text-left text-emerald-300">Restore</button>:<button onClick={()=>remove(resource,item)} className="text-left text-rose-300">Archive</button>}</div>})}</div>:<p className="mt-5 rounded-xl border border-white/[.06] bg-white/[.025] p-6 text-sm text-white/45">No records yet. Add one from the button above.</p>}</div><div className="glass rounded-xl p-5"><p className="text-sm">Module actions</p>{active==="Media"?(["gallery","sponsors","achievements"] as Resource[]).map((resource)=><button onClick={()=>open({resource,title:`Add ${resource}`,fields:extraFields[resource]})} className="mt-3 block w-full rounded-xl border border-white/[.07] bg-white/[.035] px-4 py-3 text-left text-xs text-white/60 transition hover:bg-violet-500/[.08]" key={resource}>Add {resource}</button>):<button onClick={()=>open({resource:c.resource,title:`Add ${active.slice(0,-1)}`,fields:c.fields})} className="mt-3 block w-full rounded-xl border border-white/[.07] bg-white/[.035] px-4 py-3 text-left text-xs text-white/60 transition hover:bg-violet-500/[.08]">Create record</button>}<p className="mt-3 rounded-xl border border-white/[.07] bg-white/[.035] px-4 py-3 text-xs text-white/55">{active==="Teams"?"Active teams appear in member dropdowns. Restore inactive teams before assigning members.":"Archive uses safe public removal."}</p></div></div>}
