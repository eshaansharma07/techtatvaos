  ...
  return <div className="mt-7 grid gap-4 xl:grid-cols-[1fr_.36fr]">
    <div className="grid gap-4">
      ...
      <div className="glass rounded-xl p-5">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-[1fr_auto_auto_auto_auto_auto]"><label className="portal-search flex items-center gap-3 rounded-2xl px-4 py-3 text-white/40"><Search size={14}/><input value={query} onChange={(e)=>{setQuery(e.target.value);setPage(0);}} placeholder="Search name, email, UID..." className="w-full bg-transparent text-sm text-white outline-none placeholder:text-white/25"/></label><select value={statusFilter} onChange={(e)=>{setStatusFilter(e.target.value);setPage(0);}} className="rounded-2xl border border-white/[.08] bg-black/20 px-4 py-3 text-xs text-white/70"><option value="all">All statuses</option><option value="pending">Pending</option><option value="shortlisted">Shortlisted</option><option value="accepted">Accepted</option><option value="rejected">Rejected</option><option value="on_hold">On hold</option></select><select value={teamFilter} onChange={(e)=>{setTeamFilter(e.target.value);setPage(0);}} className="rounded-2xl border border-white/[.08] bg-black/20 px-4 py-3 text-xs text-white/70"><option value="all">All teams</option>{teams.map((team:any)=><option key={idOf(team)} value={idOf(team)}>{team.name}</option>)}</select><select value={roleFilter} onChange={(e)=>{setRoleFilter(e.target.value);setPage(0);}} className="rounded-2xl border border-white/[.08] bg-black/20 px-4 py-3 text-xs text-white/70"><option value="all">All roles</option>{roles.map((role:any)=><option key={idOf(role)} value={idOf(role)}>{role.name}</option>)}</select><input value={dateFrom} onChange={(e)=>{setDateFrom(e.target.value);setPage(0);}} type="date" className="rounded-2xl border border-white/[.08] bg-black/20 px-4 py-3 text-xs text-white/70"/><input value={dateTo} onChange={(e)=>{setDateTo(e.target.value);setPage(0);}} type="date" className="rounded-2xl border border-white/[.08] bg-black/20 px-4 py-3 text-xs text-white/70"/></div>
