...
function teamNamesOf(member:any){const names=(member.teams||[]).map((team:any)=>team?.name).filter(Boolean);return names.length?names.join(", "):(member.team?.name || "")}
function memberLabel(member:any){const teams=teamNamesOf(member);return `${member.name || "Unnamed member"}${teams ? ` / ${teams}` : ""}`}
function leadRolesOf(member:any,teams:any[]){const userId=idOf(member);return (teams||[]).filter((team:any)=>idOf(team.lead)===userId || (team.coLeads||[]).some((lead:any)=>idOf(lead)===userId)).map((team:any)=>`${idOf(team.lead)===userId?"Lead":"Co-lead"}: ${team.name}`).join(", ")}
